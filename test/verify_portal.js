...
var communitiesCss = fs.readFileSync(
    path.join(portal, "style", "olv_plaza_concept.css"),
    "utf8"
);
...
ok(html.indexOf('style/olv_plaza_concept.css') >= 0, "Plaza Lite CSS is not loaded");
...
ok(html.indexOf('src="res/olv/commu/thumb/MarioParty10.png"') >= 0, "Optimized Mario Party thumbnail is not used");
...
ok(html.indexOf('class="wut-community-demo-strip"') < 0, "Legacy Plaza demo strip is still present");
...
ok(communitiesCss.indexOf("width: 96px") >= 0, "Plaza community artwork sizing is missing");
ok(communitiesCss.indexOf("width: 790px") >= 0, "Free-flow Plaza community column is missing");
ok(communitiesCss.indexOf("wut-plaza-native-page") >= 0, "Native document Plaza mode is missing");
ok(communitiesJs.indexOf(".scrollTop") < 0, "Communities controller must not use an internal scrollTop scroller");
ok(communitiesJs.indexOf("link.focus()") < 0, "Communities controller must not rely on focus auto-scroll");
